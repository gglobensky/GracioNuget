...
export class Gracio {
  ...
  static fromFloat(value: number | string): Gracio {
    ...
    if (eIndex !== -1) {
      ...
      if (isNaN(exponent)) throw new Error(`Invalid exponent in float: ${s}`);
  ...
  private lastSimplifiedBits = 0;

  public shouldSimplify(): boolean {
  ...
  private checkPrecisionLimit(): void {
  ...
  public simplify(): this {
    ...
    if (a === 0n) return this;
    if (b === 0n) throw new Error("Denominator cannot be zero");
  ...
  add(other: Gracio): this {
    ...
    if (this.shouldSimplify()) this.simplify();
  ...
  private static integerRoot(value: bigint, index: bigint): bigint | null {
    if (value === 0n) return 0n;
    if (value < 0n && index % 2n === 0n) return null; // Even root of negative
    ...
    for (const p of SMALL_PRIMES) {
      if (absVal % p === 0n) {
        ...
        if (count % Number(index) !== 0) return null;
      }
    }
    ...
    while (low <= high) {
      ...
      const p = mid ** index;
      if (p === absVal) return isNegative ? -mid : mid;
      if (p < absVal) low = mid + 1n;
      else high = mid - 1n;
    }
    return null;
  }
