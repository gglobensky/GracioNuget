...
type ParserResult = 
  ...
  | { type: 'root', index: bigint, value: { n: bigint, d: bigint } };

export class Parser {
  private position = 0;
  private tokens: Token[];
  private logger?: (message: string) => Promise<void> | void;

  constructor(tokens: Token[], logger?: (message: string) => Promise<void> | void) {
    this.tokens = tokens;
    this.logger = logger;
  }
  ...
  private collapse(res: ParserResult): { n: bigint; d: bigint } {
    if (res.type === 'ratio') return { n: res.n, d: res.d };
  ...
  private parseExpression(): ParserResult {
  ...
  private parseTerm(): ParserResult {
  ...
  private parseExponent(): ParserResult {
  ...
  private parseFactor(): ParserResult {
    ...
    if (token.type === 'function' && token.value === 'root') {
      ...
      if (this.currentToken().type !== '(') throw new Error("Expected '(' after root");
      this.position++; // skip '('
      ...
      if (this.currentToken().type !== ')') throw new Error("Expected ')' after root arguments");
      this.position++; // skip ')'
    ...
    if (token.type === '(') {
      this.position++; // skip '('
      const result = this.parseExpression();
      if (this.currentToken().type !== ')') throw new Error("Expected closing parenthesis");
      this.position++; // skip ')'
      return result;
    }
